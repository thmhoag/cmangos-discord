...
func (router *Router) RegisterCmd(cmd *Command) {

	if cmd.RateLimiter == nil {
		cmd.RateLimiter = NewRateLimiter(1*time.Second, 1*time.Second, func(ctx *Ctx) {
			author := ctx.Event.Author
			log.Printf("user %s (%s#%s) is being rate limited", author.ID, author.Username, author.Discriminator)
		})
	}
